...
async function goCheckout() {
  ...
  // Геокодируем адрес если зона ещё не определена
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        if (zone) {
          ...
          s.style.display = 'block'; s.style.color = 'green';
          s.textContent = '✅ Доставка: ' + zone.price;
        }
      }
    } catch(e) { console.log('geocode error:', e); }
  }

  // Грузим сохранённую карту параллельно
  savedCard = await loadSavedCard();
